      ...
      <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
        ...
        <div>
          <p className='mt-4 text-gray-800'>
            Prince  <span className='text-xs'>/night</span>
          </p>
          <input onChange={e=>setInputs({...inputs,pricePerNight:e.target.value})}
           type = "number" placeholder='0' className='border border-gray-300 mt-1 rounded p-2 w-24' value = {inputs.pricePerNight}/>
        </div>
       </div>
       ...
       <div className='flex flex-col flex-wrap mt-1 text-gray-400 max-w-sm'>
        {Object.keys(inputs.amenities).map((amenity,index)=>(
          ...
          <div key = {index}>
            <input 
            type = "checkbox" id= {`amenities${index+1}`} checked={inputs.amenities[amenity]}
            onChange={()=>setInputs({...inputs,amenities:{
